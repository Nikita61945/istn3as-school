Public Class EditTimeTable
    ...
    Private Sub EditTimeTable_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Dim value As Integer = CreateTimetable.genWithGrade

        If Not (value = 0) Then
            GradeComboBox.DropDownStyle = ComboBoxStyle.DropDown
            openWithGrade(value)
            GradeComboBox.SelectedValue = "Grade " + value.ToString()
        End If
        ...
        If SQL.HasConnnection = True Then
            ...
            If SQL.SQLDataSet.Tables(0).Rows.Count > 0 Then
                For x As Integer = 0 To 4
                    SQL.RunQuery("SELECT Day, P1, P2, P3, P5, P4, P6, P7,P8,P9 FROM TimeTable WHERE (Grade = '" & newGrade & "') AND (Day = '" & CreateTT.Rows(x).Cells(0).Value & "') ")

                    If SQL.SQLDataSet.Tables(0).Rows.Count > 0 Then
                        For Each k As DataRow In SQL.SQLDataSet.Tables(0).Rows
                            For y As Integer = 1 To 9
                                CreateTT.Rows(x).Cells(y).Value = SQL.SQLDataSet.Tables(0).Rows(0).Item(y)
                                CreateTT.Rows(x).Cells(y).Style.BackColor = Color.LightGray
                            Next
                        Next
                    End If
                ...
                GenerateButtons()
            Else
                MessageBox.Show("Please create a new timetable for grade " + newGrade.ToString() + ".", "No Timetable Found.")
        ...
        If Not (saved) Then
            If MessageBox.Show("If you leave now all data will be lost." + vbNewLine + "Do you want to save your current data?", "Error", MessageBoxButtons.YesNo) = DialogResult.Yes Then
                If SQL.HasConnnection = True Then
                    ...
                    For x As Integer = 0 To CreateTT.Rows.Count - 1 Step +1
                        SQL.RunQuery("UPDATE TimeTable SET P1 = '" & CreateTT.Rows(x).Cells(1).Value & "', P2 = '" & CreateTT.Rows(x).Cells(2).Value & "', P3 = '" & CreateTT.Rows(x).Cells(3).Value & "', P4 = '" & CreateTT.Rows(x).Cells(4).Value & "', P5 = '" & CreateTT.Rows(x).Cells(5).Value & "', P6 = '" & CreateTT.Rows(x).Cells(6).Value & "', P7 = '" & CreateTT.Rows(x).Cells(7).Value & "', P8 = '" & CreateTT.Rows(x).Cells(8).Value & "', P9 = '" & CreateTT.Rows(x).Cells(9).Value & "' WHERE (Day =  '" & CreateTT.Rows(x).Cells(0).Value & "') AND (Grade ='" & Grade & "')")
                    Next
    ...
    Private Sub SaveBtn_Click(sender As Object, e As EventArgs) Handles SaveBtn.Click
        If MessageBox.Show("Do you want to save changes made to the grade " + Grade.ToString + " timetable?", "Update Confirmation", MessageBoxButtons.YesNo) = DialogResult.Yes Then
            ...
            If SQL.HasConnnection = True Then
                ...
                For x As Integer = 0 To CreateTT.Rows.Count - 1 Step +1
                    SQL.RunQuery("UPDATE TimeTable SET P1 = '" & CreateTT.Rows(x).Cells(1).Value & "', P2 = '" & CreateTT.Rows(x).Cells(2).Value & "', P3 = '" & CreateTT.Rows(x).Cells(3).Value & "', P4 = '" & CreateTT.Rows(x).Cells(4).Value & "', P5 = '" & CreateTT.Rows(x).Cells(5).Value & "', P6 = '" & CreateTT.Rows(x).Cells(6).Value & "', P7 = '" & CreateTT.Rows(x).Cells(7).Value & "' , P8 = '" & CreateTT.Rows(x).Cells(8).Value & "', P9 = '" & CreateTT.Rows(x).Cells(9).Value & "' WHERE (Day =  '" & CreateTT.Rows(x).Cells(0).Value & "') AND (Grade ='" & Grade & "')")
                Next
                MessageBox.Show("Timetable Updated!", "Confirmation")
                saved = True
            End If
        End If
    End Sub

    Public Sub openWithGrade(OGrade As Integer)
        CreateTT.Enabled = True
        ...
        Dim newGrade As Integer = OGrade
        If SQL.HasConnnection = True Then
        ...
        End If
    End Sub
    Private Sub EditTT_CellDoubleClick(sender As Object, e As DataGridViewCellEventArgs) Handles CreateTT.CellDoubleClick
